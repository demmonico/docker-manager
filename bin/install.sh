...
if [ ! -z "$( netstat -ln | grep ":80 " )" ] && [ ! -z "$( dpkg --get-selections | grep apache )" ]; then
...
# server environment
    ...
    echo -e "${YELLOW}Info:${NC} updating repo info ... "
    VERSION_DOCKER_COMPOSE='1.17.0'
    sudo apt-get update && \
        echo -e "${YELLOW}Info:${NC} check for installs ... " && \
...
echo -e "${GREEN}Now you can see your projects from Docker Manager as${NC} ${YELLOW}http://${HOST_NAME}/${NC} ${GREEN}and sub-domains${NC}";
